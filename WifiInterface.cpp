...
// NMCK: Commented out following block as it isn't relevant to me and slows down the startup.
/*NMCK*/
...
/*NMCK*/
  ...
  if (!ipOK) {
    ...
    checkForOK(1000, OK_SEARCH, true, false); // Not always OK, sometimes "no change"

    // Figure out MAC addr
    StringFormatter::send(wifiStream, F("AT+CIFSR\r\n"));
    // looking fpr mac addr eg +CIFSR:APMAC,"be:dd:c2:5c:6b:b7"
    if (checkForOK(5000, (const char*) F("+CIFSR:APMAC,\""), false)) {
    ...
    // NMCK: Added following line to remove OK from buffer.
    /*NMCK*/ checkForOK(1000, OK_SEARCH, false); /*NMCK*/
    ...
    if (oldCmd) {
      ...
      int i=0;
      //do StringFormatter::send(wifiStream, F("AT+CWSAP=\"DCCEX_%s\",\"PASS_%s\",10,3.8,0\r\n"), macTail, macTail);
      do StringFormatter::send(wifiStream, F("AT+CWSAP=\"%S\",\"%S\",10,3,8,0\r\n"), SSid, password);
      while (i++<2 && !checkForOK(16000, OK_SEARCH, true, false)); // do twice if necessary but ignore failure as AP mode may still be ok
      
    } else {

      //StringFormatter::send(wifiStream, F("AT+CWSAP_CUR=\"DCCEX_%s\",\"PASS_%s\",10,3,8,0\r\n"), macTail, macTail);
      StringFormatter::send(wifiStream, F("AT+CWSAP_CUR=\"%S\",\"%S\",10,3,8,0\r\n"), SSid, password);
...
void WifiInterface::ATCommand(const byte * command) {
  ...
  if (*command=='X') {
     ...
     DIAG(F("\n++++++ Wifi Connection forced on ++++++++\n"));
        ...
        checkForOK(10000, OK_SEARCH, true, false);
  }
}
